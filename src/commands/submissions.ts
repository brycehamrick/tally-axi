...
import { pageCountLine, renderResult, type AxiRenderable } from "../lib/output.js";
import { truncateStringsDeep, truncationHint } from "../lib/truncate.js";
import type { CommandContext } from "../context.js";
import type { Submission } from "../tally/models.js";
...
export async function submissionsCommand(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  const sub = args[0];
...
async function submissionsList(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  const rows = result.items.map((submission) =>
    compact({
  ...
  if (rows.length === 0) {
    ...
    if (firstId) help.push(`Run \`tally-axi submissions get ${formId} ${firstId}\` for the full answers`);
  }
  const paging = pageCountLine(rows.length, result.total, page, limit);
  if (paging) help.push(paging);
  out["help"] = help;
  return renderResult(out, json);
}

async function submissionsGet(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  const submission: Submission = await ctx.client.getSubmission(formId, submissionId);
  const help = [
...
async function submissionsDelete(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
