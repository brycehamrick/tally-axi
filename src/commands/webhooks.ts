...
import { renderResult, type AxiRenderable } from "../lib/output.js";
import { snippet } from "../lib/truncate.js";
import type { CommandContext } from "../context.js";
import type { Webhook } from "../tally/models.js";
...
const EVENTS = ["FORM_RESPONSE"] as const;

export async function webhooksCommand(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  const sub = args[0];
...
async function webhooksList(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  const webhooks: Webhook[] = await ctx.client.listWebhooks(formId);
...
async function webhooksCreate(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  const webhook: Webhook = await ctx.client.createWebhook({ formId, url, event });
  return renderResult(
    {
      webhook: compact({
        id: asString(webhook.id),
...
async function webhooksDelete(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
