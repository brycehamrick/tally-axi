...
import { forbidExtraPositionals, oneOf, optionalInt, parseFlags, requirePositional, type FlagDefinition } from "../lib/args.js";
...
import type { Form } from "../tally/models.js";
...
export async function formsCommand(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  const sub = args[0];
...
async function formsList(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  const rows = result.items.map((form) =>
    compact({
      id: asString(form.id),
  ...
  if (rows.length === 0) {
    ...
    if (firstId) help.push(`Run \`tally-axi forms get ${firstId}\` for the form definition`);
...
async function formsGet(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  const form: Form = await ctx.client.getForm(formId);
  const help = [
