...
export const DESCRIPTION =
  "Tally forms, submissions, and webhooks for agents - token-efficient reads over the Tally REST API";
 ...
 */
export function toAxiError(error: unknown): unknown {
  if (!(error instanceof TallyError)) return error;
  const suggestions: string[] = [];
  if (error.status !== undefined) suggestions.push(`Tally responded with HTTP ${error.status}`);
  if (error.retryAfterMs !== undefined && error.retryAfterMs > 0) suggestions.push(`Retry after ${Math.ceil(error.retryAfterMs / 1000)}s`);
  if (error.code === "AUTHENTICATION_FAILED") suggestions.push("Create or rotate the key in Tally settings, then update TALLY_API_KEY");
  if (error.code === "NOT_FOUND") suggestions.push("Run `tally-axi forms list` to confirm the id");
