...
export function parseFlags(argv: string[], commandPath: string, flags: Record<string, FlagDefinition>): ParsedFlags {
  try {
...
export function requireString(values: FlagValues, name: string, commandPath: string): string | undefined {
...
export function requiredString(values: FlagValues, name: string, commandPath: string): string {
...
export function optionalInt(values: FlagValues, name: string, opts: { min: number; max: number }): number | undefined {
  const value = values[name];
  if (value === undefined) return undefined;
...
export function oneOf<T extends string>(values: FlagValues, name: string, allowed: readonly T[]): T | undefined {
  const value = values[name];
  if (value === undefined) return undefined;
...
export function requireHttpsUrl(raw: string, label: string): string {
  let parsed: URL;
  try {
    parsed = new URL(raw);
  } catch {
...
export function requirePositional(positionals: string[], index: number, label: string, commandPath: string): string {
...
export function forbidExtraPositionals(positionals: string[], expected: number, commandPath: string): void {
...
export function requireConfirm(values: FlagValues, commandPath: string, preview: string): void {
