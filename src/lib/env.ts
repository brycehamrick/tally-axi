...
export interface TallyConfig {
  /** Bearer token for the Tally REST API. Absent until the user configures one. */
  apiKey?: string;
  /** HTTPS-only REST endpoint. */
  apiBaseUrl: string;
  /** Per-request timeout in milliseconds (100-120000). */
  timeoutMs: number;
}
...
export function resolveConfig(env: NodeJS.ProcessEnv = process.env): TallyConfig {
...
export function ensureApiKey(config: TallyConfig, commandPath: string): string {
  if (!config.apiKey) {
